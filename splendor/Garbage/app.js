...
var game = function () {
    ...
    this.buyCards = function (player, cardLvl, cardId) { // cardLvl(1-3) decks (4-7)
        player.payCard(this.deck[cardLvl + 4][cardId]);
        player.addCard(this.deck[cardLvl + 4].splice(cardId, 1)[0]);
    };
};


//Build Master Deck from JSON file.
...
<!-- Base scripts to connect and keep track of connections -->
...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('signIn', function (data) {
        ...
        splendor.playerList[socket.id] = new player(socket.name);
